...
def Min_x(z_k,y_k,Qx,Qz,A,B,c,rho):
    ...
    z = z_k
    y = y_k
    L_lambda = cp.quad_form(x,Qx) + cp.quad_form(z,Qz) + y.T @ (A @ x + B @ z - c) + (rho/2)*cp.sum_squares(A @ x + B @ z - c)
    prob = cp.Problem(cp.Minimize(L_lambda))
    prob.solve()
    x_next = x.value
...
for i in range(N):
    ...
    # Updates for decision variables
